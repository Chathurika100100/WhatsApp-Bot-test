...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async m => {
        ...
        // ═══════════════════════════════════════════════════════
        // 📋 Reply to FitGirl PARTS List (Download Commands)
        // ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// 🔍 FIXED: Get Direct Download Link using Puppeteer
// ═══════════════════════════════════════════════════════════════
async function getDirectDownloadLink(fuckingFastUrl) {
    let browser = null;
    try {
        ...
        console.log(`[DEBUG] Launching browser for: ${fuckingFastUrl}`);
        ...
        await page.goto(fuckingFastUrl, { 
        ...
        // Extract the direct download link from the page
        console.log(`[DEBUG] Extracting direct link...`);
        const directLink = await page.evaluate(() => {
            // Method 1: Look for window.open with dl.fuckingfast.co
            const scripts = document.querySelectorAll('script');
            for (const script of scripts) {
                const text = script.textContent || '';
                const match = text.match(/window\.open\s*\(\s*["'](https:\/\/dl\.fuckingfast\.co\/dl\/[^"']+)["']/);
                if (match) return match[1];

                // Method 2: atob decoded
                const atobMatch = text.match(/window\.open\s*\(\s*atob\s*\(\s*["']([A-Za-z0-9+/=]+)["']\s*\)/);
                if (atobMatch) {
                    try {
                ...
                // Method 3: Any dl.fuckingfast.co URL
                const dlMatch = text.match(/https:\/\/dl\.fuckingfast\.co\/dl\/[^"'\s]+/);
                if (dlMatch) return dlMatch[0];
            }

            // Method 4: Check entire HTML
            const html = document.documentElement.innerHTML;
            const htmlMatch = html.match(/https:\/\/dl\.fuckingfast\.co\/dl\/[^"'\s]+/);
            if (htmlMatch) return htmlMatch[0];

            return null;
        });

        console.log(`[DEBUG] Direct link found: ${directLink ? directLink.substring(0, 60) + '...' : 'NULL'}`);

        await browser.close();
        return directLink;

    } catch (error) {
        console.error('[DEBUG] Puppeteer Error:', error.message);
...
async function handleFitGirlDownload(sock, msg, linksToSend, sendToJid, chatJid, session, isGroup = false) {
    ...
    if (uploadedCount > 0 && !wasStopped) {
        ...
        await sock.sendMessage(chatJid, { 
            text: `❌ කිසිදු part එකක් download කිරීමට නොහැකි විය. Puppeteer/Cloudflare issue.`,
            edit: progressMsg.key 
        });
    }
